...
public final class SqlSessionUtils {
  ...
  private static void registerSessionHolder(SqlSessionFactory sessionFactory, ExecutorType executorType,
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
      ...
      if (environment.getTransactionFactory() instanceof SpringManagedTransactionFactory) {
        ...
        // 判定到 TransactionSynchronizationManager 中
  ...
  public static void closeSqlSession(SqlSession session, SqlSessionFactory sessionFactory) {
    ...
    // 从 TransactionSynchronizationManager 中获得 SqlSessionHolder 对象
    SqlSessionHolder holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);
    // 如果相等，说明在 Spring 托管的事务中，则释放 holder 计数
    if ((holder != null) && (holder.getSqlSession() == session)) {
      LOGGER.debug(() -> "Releasing transactional SqlSession [" + session + "]");
      holder.released();
      // 如果不相等，则直接关闭SqlSession
    } else {
  ...
  public static boolean isSqlSessionTransactional(SqlSession session, SqlSessionFactory sessionFactory) {
    ...
    // 从 TransactionSynchronizationManager 中获得SqlSessionHolder对象
    SqlSessionHolder holder = (SqlSessionHolder) TransactionSynchronizationManager.getResource(sessionFactory);
  ...
  private static final class SqlSessionSynchronization extends TransactionSynchronizationAdapter {
    ...
    /**
     * 提交事务完成之前， 关闭session对象 在beforeCommit执行之后执行，commit执行后，可以关闭SqlSession
     */
    @Override
